...
discovering(timeout,State) when State#state.sn==0 ->
  ...
  NewState =  prepare_for_phase_1(State,timeout),
  {next_state,discovering, NewState};
...
discovering(Event,State) when Event==timeout; Event==rd_empty ->
  ...
  if Nrs_new == [] ->
    Result = check_phase1_exp(State#state.exp_counter,State#state.sn),
    case Result of
      finish ->
        log:info("[~p]************** FINISHED EXPERIMENT *******************~n",[?MODULE]),
        {stop,{shutdown,{done_experiment_number,?EXP_ID}},State};
      reinitialize ->
        log:info("[~p]~~~~~~~~~~~~FINISHED EXPERIMENT ITERATION. ~p, STARTING OVER ~~~~~~~~~~~~~~~~~~~~~n",
      ...
      phase2 ->
        ...
        NewState = prepare_for_phase_2(State,Event),
        {next_state, collecting, NewState};
      naive ->
        log:info("[~p]=========FINISHED round ~p on naive, preparing for next ========~n",
          [?MODULE,State#state.sn]),
        app_utils:report_next_session(State#state.sn+1,[]),
        NewState = prepare_for_phase_1(State,Event),
        {next_state, discovering, NewState}
    end;
    true ->
      case Event of
...
collecting(rd_empty,State) ->
  ...
  if Ter8 == [] ->
    ...
    case Result of
      finish -> log:info("[~p]************** FINISHED EXPERIMENT *******************~n",[?MODULE]),
        {stop,{shutdown,{done_experiment_number,?EXP_ID}},State};
      reinitialize ->
        log:info("[~p]~~~~~~~~~~~~FINISHED EXPERIMENT ITERATION. ~p, STARTING OVER ~~~~~~~~~~~~~~~~~~~~~n",
          [?MODULE, State#state.exp_counter]),
      ...
      phase2 ->
        log:info("[~p]======== FINISHED ROUND ~p of collecting, preparing for next round =======~n",
          [?MODULE,State#state.sn]),
        app_utils:report_next_session(State#state.sn+1,State#state.ter),
        NewState = prepare_for_phase_2(State,rd_empty),
        {next_state, collecting, NewState}
    end;
    true ->
    ...
    case Result of
      ...
      phase2 ->
        log:info("[~p]======== FINISHED ROUND ~p of collecting, preparing for next round =======~n",
          [?MODULE,State#state.sn]),
        app_utils:report_next_session(State#state.sn+1,State#state.ter),
        NewState = prepare_for_phase_2(State,timeout),
        {next_state, collecting, NewState}
    end;
    true ->
...
terminate(Reason, StateName, State) ->
  ...
  case Reason of
    {shutdown, {done_experiment_number,?EXP_ID}} ->
      log:debug("[~p] handling end of experiment number ~p~n",[?MODULE,?EXP_ID]),
      app_utils:export_db("exp_"++ ?EXP_ID);
    Reason ->[],
...
check_phase1_exp(CurrentExp, CurrentSession) when ?AMR_MODE == naive ->
  if CurrentSession-1 == ?PHASE1_COUNT ->     % finished required times of phase 2 per experiment
    ...
    true -> naive                            % didn't finish current experiment yet
  end;



check_phase1_exp(CurrentExp,_CurrentSession) ->
...
prepare_for_phase_1(State,Event)->
  ...
  app_utils:update_tracker_requests(Rd,Sn),
  if ?AMR_MODE==naive->
...
prepare_for_phase_2(State,Event)->
  if Event == rd_empty -> State#state.timer!stop; true->[]  end,
  app_utils:report_averages(State#state.sn),
  app_utils:insert_nodes_to_tracker(State#state.meters),
  Sn = State#state.sn +1,
