...
loop_send(Channel, N, S) -> 
		receive after 500*S-> ?LOGGER:debug("modem_port: sending.....~n"),
		X = random:uniform(2),
		if X == 1 -> Y = 16; true-> Y=36 end,
		
		Payload = [N rem 255] ++ lists:seq(15, 15+Y),
		?LOGGER:debug("modem_port: loop_send: size of payload is:~p~n", [length(Payload)]),
		send(Channel), 
...
supervise() ->
	receive
		...
		%modem port terminated normaly, allow it and exit supervisor with normal reason
		 {'DOWN', _Ref, process, _Pid, normal} -> ?LOGGER:debug("modem_port: terminated normally~n"), 1;%exit(normal);
		%modem port terminated for unknown reason, return 1 for incrementing port's crashes 
		 {'DOWN', _Ref, process, _Pid, port_terminated} -> ?LOGGER:debug("modem_port: terminated because of some error11~n"), 1;
		%modem port terminated due to too many errors (unsent packets or damaged packets), return 1 for incrementing port's crashes 
		 {'DOWN', _Ref, process, _Pid, port_errors} -> ?LOGGER:debug("modem_port: terminated because of port errors22~n"), 1;
	   	{'DOWN', _Ref, process, _Pid, Else} ->?LOGGER:debug("modem_port: terminated unknown reason: ~p.~n", [Else]), 1
	end.

%in case too many port crashes occured, terminate
init_supervisor( _Caller_PID, Port_crash) when Port_crash =:= ?PORTS_CRASH_LIMIT -> 
	?LOGGER:debug("modem_port: modem_port:init_supervise: too many port crashes!!!! wow!!!! something is really not working~n"),
...
init(Super_PID) ->
    ...
    compile_c_port(),
    os:cmd("sh " ++ ?PROGRAM_PATH ++ "uart_config.sh"),
    ...
    X = os:timestamp(),
	random:seed(X),
    ?LOGGER:debug("modem_port: done init~n"),
...
loop(_, OS_PID ,_,Port_Errors) when Port_Errors =:= ?PORTS_ERRORS_LIMIT -> 
	?LOGGER:debug("modem_port: too many port errors. terminate myself!!!!~n"),
...
loop(Port, OS_PID, Super_PID, Port_Errors) ->
	...
	{call, Msg} ->
			MSG = preper_msg(Msg),
			if is_list(MSG) -> sendMsg(Port, MSG); true -> ?LOGGER:debug("modem_port: error prepering msg ~p~n", [MSG]),dont_send end, 
	...
	{stop, Reason} ->
	    ...
	    close_c_port_program(OS_PID),
	    receive
		{Port, closed} -> ?LOGGER:debug("modem_port: %%%%%%%~n~n"), exit(Reason)
	    end;
	%c port terminates unexpectedly
	{'EXIT', _Port, Reason} ->
		?LOGGER:debug("modem_port: got exit from c port. reason is:~p.~n", [Reason]),
...
exemine_data( [Channel, RSSI | Rest]) -> 
	...
	?LOGGER:debug("modem_port: CRC is:~p~n", [CRC]),
	CRC2 = erlang:crc32(L_NO_CRC),
	?LOGGER:debug("modem_port: CRC2 is:~p. L_NO_CRC is:~p~n", [CRC2, L_NO_CRC]),
	case CRC2  of
...
compile_c_port() ->
		...
		?LOGGER:debug("modem_port: compile C port command : ~p~n", [S]),
		_STR = os:cmd(S),
...
close_c_port_program(OS_PID)->
		...
		?LOGGER:debug("modem_port: closed c program for real~n").
...
sendMsg(Port, MSG) ->
	[H|T] = MSG,
	backoff(H),
	?LOGGER:debug("modem_port: \tMSG is:~p~n~n", [MSG]),
...
preper_msg([Channel, _Reserved | _Rest]) when (Channel > 3) orelse (Channel<0) -> ?LOGGER:debug("modem_port: bad channel~p~n",[Channel]),ignore_msg;
preper_msg([Channel, _ | Rest]) -> Size = lists:flatlength(Rest), 
	case Size of
		S1 when S1 =< 14 -> X = 20 - Size - 4 - 2, if X > 0 -> PAD = lists:seq(1,X); true -> PAD = [] end ,
						    ...
						    ?LOGGER:debug("modem_port: LCRC is:~p~n,", [LCRC]),
							[Channel] ++ [0] ++ L ++ LCRC;
...
backoff(Channel)  when Channel =:=1 ->
	...
	?LOGGER:debug("modem_port: ~nbackoff - (~p)", [Wait]),
	receive after Wait -> ready end;
backoff(Channel) ->
    ?LOGGER:debug("modem_port: BACKOFF on channel ~p~n", [Channel]),
    plc_has_backoff.
