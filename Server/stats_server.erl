...
start_link() ->
    io:format("Starting stats_server~n"),
...
init([]) ->
  io:format("initiating stats_server~n"),
...
-spec(handle_cast(Request :: term(), State :: #state{}) ->
  ...
  handle_cast({states, From}, State) ->
      ...
      AvgTime = avrageTime(State#state.dm_ets),
      AvgLength = 0,
...
%  io:format("stats_server got report about: Incoming data msg ~p from ~p to ~p at ~p sent at ~p~n",[Id, Source,Destination, UTIME,StatrTIME]),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
handle_cast({{data_message, relay_message}, Data}, State = #state{dm_ets = DM_ets, db = DB, counters = Counters}) ->
...
%%%===================================================================
avrages(DB) ->
    avrages(DB, ets:first(DB)).

avrages(DB, '$end_of_table') -> {0,0};
avrages(DB, Key) -> {0,0}.
%[{Id,{StartTime,EndTime ,Relays}}] = ets:lookup(DM_ets,Id),
