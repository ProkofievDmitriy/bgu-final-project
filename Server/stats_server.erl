...
handle_cast(stop, State) ->
  ...
  handle_cast({Type, Data}, State = #state{dm_ets = DM_ets, db = DB, counters = Counters}) ->
    ...
    dets:insert(DB, {UTIME, Type , Data}),
...
update_dm_ets(DM_ets, relay, Id, _UTIME) ->
  ...
  case Curr of
    ...
    A -> io:format("~n~nSHOULD NEVER GET IN - CASE CLAUSE FAILED, if you see this there is a problem node should not send ~p now~n~n",[A]),
         ok
  end;
...
average(DB, Key, SumTime, NumberTime, SumLength, NumberLength) ->
  ...
  case Data of
    ...
    %  io:format("Line 414: ~p",[Data]),
      average(DB, ets:next(DB,Key), SumTime + (EndTime - StartTime), NumberTime+1, SumLength + Relays+1, NumberLength+1);
    _ ->
