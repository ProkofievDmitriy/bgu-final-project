...
-define(LOCAL_SERVER(Record), Record#soldier_p.local_server).
-define(TARGET_X(Record),Record#soldier_p.target_x).
-define(TARGET_Y(Record),Record#soldier_p.target_y).
-define(TARGET_PID(Record),Record#soldier_p.target_pid).
-define(HP(Record),Record#soldier_p.hp).
-define(RADIUS,60).
-define(ANGLE, 360).
...
init([UnitRecord, LocalServer]) ->
	UnitState = case UnitRecord#params.state of
					%% First Initial
			undefined -> [X,Y] = UnitRecord#params.location, %% Get Location
						 UnitRecord#params{type=soldier,  %% Initial Record
										  others=#soldier_p{hp=?HP, local_server=LocalServer, target_x=X, target_y=Y},
										  direction = mymath:get_random(359),
										  see_range={?RADIUS,?ANGLE},
										  state = seek};
					%% Save State
			_Else -> UnitRecord#params{others=(UnitRecord#params.others)#soldier_p{local_server=LocalServer},
...
seek(update_location, StateData) ->
	...
	Params = (StateData#params.others)#soldier_p{target_x=NewTargetX, target_y=NewTargetY, target_pid=undefined},
	%% New State
...
seek({damage_received, Damage}, StateData) ->
	...
	Params = (StateData#params.others)#soldier_p{hp=NewHP},
...
seek({target_found, ClosestTarget}, StateData) ->
	...
	%% Update Parameters
	Params = (StateData#params.others)#soldier_p{target_pid = TargetPID, target_x=TargetX, target_y=TargetY},
...
attack(attack_now, StateData) ->
	%% Get Target PID
	TargetPID = (StateData#params.others)#soldier_p.target_pid,
	...
	Reply = case Alive of
				...
				noproc -> [NewTargetX,NewTargetY,NewDirection] = random_next_step(StateData),
							 Params = (StateData#params.others)#soldier_p{target_pid = undefined, target_x=NewTargetX, target_y=NewTargetY},
				...
				TargetPID -> 
						 ...
						 case NewDistance =< ?RADIUS*?RADIUS of
							 false -> [TargetX,TargetY,NewDirection] = random_next_step(StateData),
									  Params = (StateData#params.others)#soldier_p{target_pid = undefined, target_x=TargetX, target_y=TargetY},
									  NewStateData = StateData#params{direction=NewDirection, state=seek, others=Params},
									  gameManager:insertUnit(?LOCAL_SERVER(StateData#params.others), self(), NewStateData),
									  gen_fsm:send_event_after(?TICK, update_location),
									  {next_state, seek, NewStateData};
							 true -> NewDirection = new_direction(NewTargetX, NewTargetY, StateData#params.location),
									 Params = (StateData#params.others)#soldier_p{target_x=NewTargetX, target_y=NewTargetY},
									 NewStateData = StateData#params{direction=NewDirection, others=Params, state=attack},
									 gameManager:insertUnit(?LOCAL_SERVER(StateData#params.others), self(), NewStateData),
...
terminate(_Reason, _StateName, StateData) ->
	%% Tell Game Manager That Unit Is Dead
...
random_next_step(StateData) ->
	...
	case NewX > ?GLOBAL_X orelse NewX < 0 orelse NewY > ?GLOBAL_Y orelse NewY < 0 of
		true -> NewStateData = StateData#params{direction = -1*NewDirection + 360},
